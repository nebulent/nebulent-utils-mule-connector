...
package com.nebulent.mule.utils.processors;
...
import com.nebulent.mule.utils.adapters.NebulentUtilsConnectorLifecycleAdapter;
...
/**
 * ThrottleMessageProcessor invokes the {@link com.nebulent.mule.utils.NebulentUtilsConnector#throttle()} method in {@link NebulentUtilsConnector }. For each argument there is a field in this processor to match it.  Before invoking the actual method the processor will evaluate and transform where possible to the expected argument type.
 * 
 */
public class ThrottleMessageProcessor
    implements FlowConstructAware, MuleContextAware, Disposable, Initialisable, Startable, Stoppable, MessageProcessor
{

    private static Logger logger = LoggerFactory.getLogger(ThrottleMessageProcessor.class);
    /**
     * Module object
     * 
     */
        ...
        throws InitialisationException
    {
        ...
        if (moduleObject == null) {
            try {
                moduleObject = muleContext.getRegistry().lookupObject(NebulentUtilsConnectorConnectionManager.class);
                if (moduleObject == null) {
                    throw new InitialisationException(MessageFactory.createStaticMessage("Cannot find object"), this);
                }
            } catch (RegistrationException e) {
                throw new InitialisationException(CoreMessages.initialisationFailure("com.nebulent.mule.utils.adapters.NebulentUtilsConnectorConnectionManager"), e, this);
            }
        }
        ...
        throws MuleException
    {
        ...
        NebulentUtilsConnectorConnectionManager _castedModuleObject = null;
        if (moduleObject instanceof String) {
            _castedModuleObject = ((NebulentUtilsConnectorConnectionManager) muleContext.getRegistry().lookupObject(((String) moduleObject)));
            if (_castedModuleObject == null) {
                throw new MessagingException(CoreMessages.failedToCreate("throttle"), event, new RuntimeException("Cannot find the configuration specified by the config-ref attribute."));
            }
        } else {
            _castedModuleObject = ((NebulentUtilsConnectorConnectionManager) moduleObject);
        }
        NebulentUtilsConnectorLifecycleAdapter connection = null;
        try {
            ...
            connection = _castedModuleObject.acquireConnection(new NebulentUtilsConnectorConnectionManager.ConnectionKey());
            if (connection == null) {
                throw new MessagingException(CoreMessages.failedToCreate("throttle"), event, new RuntimeException("Cannot create connection"));
            } else {
                if (logger.isDebugEnabled()) {
                    StringBuilder _messageStringBuilder = new StringBuilder();
            ...
            throw new MessagingException(CoreMessages.failedToInvoke("throttle"), event, e);
        } finally {
            try {
                if (connection!= null) {
                    if (logger.isDebugEnabled()) {
                    ...
                    _castedModuleObject.releaseConnection(new NebulentUtilsConnectorConnectionManager.ConnectionKey(), connection);
                }
            } catch (Exception e) {
                throw new MessagingException(CoreMessages.failedToInvoke("throttle"), event, e);
            }
